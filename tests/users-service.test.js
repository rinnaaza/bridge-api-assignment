...
describe("UsersService: authenticateUser()", () => {
  ...
  it("should throw an error when invalid externalUserId", async () => {
    await expect(
      usersService.authenticateUser({
        externalUserId: "invalid_external_user_id",
        email: "john.doe@email.com",
        password: "password123",
      })
    ).rejects.toThrow(Error);
  ...
  it("should return access_token and expires_at when uuid is provided", async () => {
    const token = await usersService.authenticateUser({
      uuid: "uuid",
      email: "john.doe@email.com",
      password: "password123",
    });
...
describe("UsersService: listUsers()", () => {
  ...
  beforeEach(() => {
    nock("https://api.bridgeapi.io")
      .get("/v2/users")
      .reply(200, {
        resources: [firstUser, secondUser, thirdUser, fourthUser],
        pagination: {
    ...
    nock("https://api.bridgeapi.io")
      ...
      .query({ limit: 2 })
      .reply(200, {
        resources: [firstUser, secondUser],
        pagination: {
          next_uri: null,
        },
      });
  ...
  it("should return an array of all users if no parameters", async () => {
    ...
    expect(users).toStrictEqual([firstUser, secondUser, thirdUser, fourthUser]);
  });
  ...
  it("should handle pagination if after parameter is provided", async () => {
    ...
    expect(users).toStrictEqual([fifthUser, sixthUser]);
  });
  ...
  it("should handle pagination and limit when limit and after parameters are provided", async () => {
    ...
    expect(users).toStrictEqual([fifthUser]);
  });
...
describe("UsersService: getUserIdByEmail()", () => {
  ...
  it("should return the id of a valid email", async () => {
    const userId = await usersService.getUserIdByEmail({
  ...
  it("should throw an error if email is not provided", async () => {
    await expect(usersService.getUserIdByEmail()).rejects.toThrow(
  ...
  it("should return null if list of users is empty", async () => {
    ...
    const httpClientForEmptyResources = new HttpClient(
      baseUrlForEmptyResources,
      clientId,
      clientSecret,
      bridgeVersion
    );
    const usersServiceForEmptyResources = new UsersService(
      httpClientForEmptyResources
    );

    const userId = await usersServiceForEmptyResources.getUserIdByEmail({
      email: "user1@email.com",
    });
