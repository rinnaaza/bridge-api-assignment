...
describe("ItemsService: listItems()", () => {
  ...
  beforeAll(() => {
    nock("https://api.bridgeapi.io", {
      ...
      .get("/v2/items")
      .reply(200, {
        resources: [firstItem, secondItem],
        pagination: {
    ...
    nock("https://api.bridgeapi.io", {
      ...
      .reply(200, {
        resources: [thirdItem, fourthItem],
        pagination: {
          next_uri: null,
        },
      });
  ...
  it("should return all the items of the first page", async () => {
    ...
    expect(items).toStrictEqual([firstItem, secondItem]);
  });
  ...
  it("should return all items when fetchAll parameter is provided", async () => {
    ...
    expect(items).toStrictEqual([firstItem, secondItem, thirdItem, fourthItem]);
  });

  it("should throw an error if accesToken is not provided", async () => {
    await expect(itemsService.listItems()).rejects.toThrow(
  ...
  it("should return list of 3 items when limit is equal to 3", async () => {
    nock("https://api.bridgeapi.io", {
      ...
      .query({ limit: 3 })
      .reply(200, {
        resources: [firstItem, secondItem, thirdItem],
        pagination: {
          next_uri: null,
        },
      });
    ...
    expect(items).toStrictEqual([firstItem, secondItem, thirdItem]);
  });
  ...
  it("should handle pagination when after parameter is provided", async () => {
    ...
    expect(items).toHaveLength(2);
    expect(items).toStrictEqual([thirdItem, fourthItem]);
  });
  ...
  it("should handle limit when limit parameter is provided", async () => {
    ...
    expect(items).toStrictEqual([firstItem]);
  });
  ...
  it("should handle limit and pagination when limit and after parameters are provided", async () => {
    ...
    expect(items).toHaveLength(1);
    expect(items).toStrictEqual([thirdItem]);
  });
  ...
  it("should return all the items if after parameter is invalid", async () => {
    nock("https://api.bridgeapi.io", {
      ...
      .reply(200, {
        resources: [firstItem, secondItem, thirdItem, fourthItem],
        pagination: {
          next_uri: null,
        },
      });
