...
describe("TransactionsService: listTransactions()", () => {
  ...
  beforeAll(() => {
    ...
    nock("https://api.bridgeapi.io", {
      ...
      .reply(200, {
        resources: [thirdTransaction, fourthTransaction],
        pagination: {
          next_uri: null,
        },
      });
  ...
  it("should return all the transactions of the first page", async () => {
    ...
    expect(transactions).toStrictEqual([firstTransaction, secondTransaction]);
  });
  ...
  it("should throw an error if accessToken is not provided", async () => {
    await expect(transactionsService.listTransactions()).rejects.toThrow(
  ...
  it("should return list of 3 items when limit is equal to 3", async () => {
    nock("https://api.bridgeapi.io", {
      ...
      .query({ limit: 3 })
      .reply(200, {
        resources: [firstTransaction, secondTransaction, thirdTransaction],
        pagination: {
          next_uri: null,
        },
      });
  ...
  it("should handle pagination when after parameter is provided", async () => {
    ...
    expect(transactions).toHaveLength(2);
    expect(transactions).toStrictEqual([thirdTransaction, fourthTransaction]);
  });
  ...
  it("should handle limit when limit parameter is provided", async () => {
    ...
    expect(transactions).toStrictEqual([firstTransaction]);
  });
  ...
  it("should handle limit and pagination when limit and after parameters are provided", async () => {
    ...
    expect(transactions).toHaveLength(1);
    expect(transactions).toStrictEqual([thirdTransaction]);
  });
  ...
  it("should return the correct transactions when until parameter is provided", async () => {
    ...
    expect(transactions).toHaveLength(1);
    expect(transactions).toStrictEqual([fourthTransaction]);
  });
  ...
  it("should return only first transaction when until and limit parameters are provided", async () => {
    ...
    expect(transactions).toHaveLength(1);
    expect(transactions).toStrictEqual([secondTransaction]);
  });
